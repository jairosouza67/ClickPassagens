...
export default function FlightCard({ resultado, onSelect }) {
  // Log para debug
  console.log('FlightCard recebeu resultado:', resultado);

  if (!resultado) {
    console.log('FlightCard: resultado é undefined');
    return null;
  }

  // Verificar se o resultado tem todas as propriedades necessárias
  if (!resultado.companhia || typeof resultado.companhia !== 'object') {
    console.error('FlightCard: resultado.companhia é inválido', {
      resultado,
  ...
  // Verificar se a companhia tem a propriedade nome
  if (!resultado.companhia.nome) {
    console.error('FlightCard: resultado.companhia.nome é undefined', {
      resultado,
      companhia: resultado.companhia
    });
    return null;
  ...
  const formatarDuracao = (duracao) => {
    if (!duracao) return '2h'; // fallback mais realista
    
    // Formato ISO 8601: PT2H30M ou PT8H0M
    const match = duracao.match(/PT(\d+)H(\d+)M/);
    if (!match) return '2h'; // fallback
        ...
        <div className="relative">
          ...
          <div className="flex items-center justify-between p-6 pb-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="font-bold text-lg text-gray-900">{resultado.companhia?.nome}</h3>
